          ...
          <table className="w-full">
            ...
            <tbody>
              {filteredBrokers.map((broker) => {
                const isInactive = broker.banned_until && new Date(broker.banned_until) > new Date()
                return (
                  <tr
                    key={broker.id}
                    onClick={() => setSelectedId(broker.id)}
                    className={`border-b hover:bg-muted/50 cursor-pointer ${isInactive ? 'opacity-60' : ''}`}
                  >
                    ...
                    <td className="p-4">
                      <span className={`px-2 py-1 rounded-full text-xs ${isInactive ? 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' : 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200'}`}>
